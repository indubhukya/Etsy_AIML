...
\begin{itemize}
  ...
  \item Random rotation ($\pm 20^{\circ}$) and colour jitter (brightness,
        contrast, saturation, hue)
  \item Random grayscale ($p=0.1$), random affine (translation, scale, shear)
...
(Eq.~\ref{eq:head}) is attached externally.  Fine-tuning unfreezes the last
three Swin stages plus the layer-normalisation head and trains for 10~epochs
(lr~$=5\times10^{-5}$), with an optional 10-epoch extension at lr~$=10^{-5}$.
...
All experiments are run in Python~3.12 with PyTorch~2.10 and \texttt{timm}~0.9
on a Google Colab T4 GPU.  Mixed-precision training (AMP) is enabled.  All
random seeds are fixed to 42 for full reproducibility.
...
lighting and minimal generative artefacts.
...
\texttt{submission.csv} with columns \texttt{image\_id} and
\texttt{ground\_truth}, matching the format of \texttt{train.csv}.
...
\textbf{Indumanaswini Bhukya:} Conceived and implemented the full pipeline,
including data loading, augmentation, ConvNeXt V2 and Swin Transformer training,
embedding fusion, meta-learner benchmarking, TTA, threshold optimisation,
Grad-CAM analysis, and final submission generation.  Wrote all sections of this
report.
